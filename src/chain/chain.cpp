...
void kth_chain_fetch_block_by_height_timestamp(kth_chain_t chain, void* ctx, uint64_t /*size_t*/ height, block_kth_hash_timestamp_fetch_handler_t handler) {
    safe_chain(chain).fetch_block_kth_hash_timestamp(height, [chain, ctx, handler](std::error_code const& ec, kth::hash_digest const& hash, uint32_t timestamp, size_t h) {
        if (ec == kth::error::success) {
...
error_code_t kth_chain_get_block_by_height_timestamp(kth_chain_t chain, uint64_t /*size_t*/ height, kth_hash_t* out_hash, uint32_t* out_timestamp) {
    ...
    safe_chain(chain).fetch_block_kth_hash_timestamp(height, [&](std::error_code const& ec, kth::hash_digest const& hash, uint32_t timestamp, size_t h) {
        if (ec == kth::error::success) {
...
void kth_chain_fetch_transaction(kth_chain_t chain, void* ctx, kth_hash_t hash, bool_t require_confirmed, transaction_fetch_handler_t handler) {
#if defined(KTH_CURRENCY_BCH)
    bool_t witness = 0;
#else
    bool_t witness = 1;
...
//void kth_chain_fetch_locator_block_hashes(kth_chain_t chain, void* ctx, get_blocks_ptr_t locator, kth_hash_t threshold, uint64_t /*size_t*/ limit, inventory_fetch_handler handler) {
//}
