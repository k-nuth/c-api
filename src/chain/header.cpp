...
uint8_t* chain_header_to_data(header_t header, uint32_t version) {
    auto const& header_cpp = *static_cast<libbitcoin::message::header const*>(header);
...
void chain_header_reset(header_t header) {
    return static_cast<libbitcoin::message::header*>(header)->reset();
}
...
uint64_t /*size_t*/ chain_header_serialized_size(header_t header, uint32_t version) {
    return static_cast<libbitcoin::message::header const*>(header)->serialized_size(version);
}
