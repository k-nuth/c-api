...
libbitcoin::chainv2::script const& chain_scriptv2_const_cpp(scriptv2_t s) {
    return *static_cast<libbitcoin::chainv2::script const*>(s);
}

libbitcoin::chainv2::script& chain_scriptv2_cpp(scriptv2_t s) {
    return *static_cast<libbitcoin::chainv2::script*>(s);
}

extern "C" {

void chain_scriptv2_destruct(scriptv2_t script) {
    auto script_cpp = static_cast<libbitcoin::chainv2::script*>(script);
    delete script_cpp;
}

int /*bool*/ chain_scriptv2_is_valid(scriptv2_t script) {
    return static_cast<int>(chain_scriptv2_const_cpp(script).is_valid());
}

int /*bool*/ chain_scriptv2_is_valid_operations(scriptv2_t script) {
    return static_cast<int>(chain_scriptv2_const_cpp(script).is_valid_operations());
}

uint64_t /*size_t*/ chain_scriptv2_satoshi_content_size(scriptv2_t script) {
    return chain_scriptv2_const_cpp(script).satoshi_content_size();
}

uint64_t /*size_t*/ chain_scriptv2_serialized_size(scriptv2_t script, int /*bool*/ prefix) {
    return chain_scriptv2_const_cpp(script).serialized_size(prefix != 0);
}
...
char const* chain_scriptv2_to_string(scriptv2_t script, uint32_t active_forks) {
    auto str = chain_scriptv2_const_cpp(script).to_string(active_forks);
...
char const* chain_scriptv2_type(scriptv2_t script){
    auto script_pattern = chain_scriptv2_const_cpp(script).pattern();
...
uint8_t const* chain_scriptv2_to_data(scriptv2_t script, int /*bool*/ prefix, uint64_t* /*size_t*/ out_size) {
    auto script_data = chain_scriptv2_const_cpp(script).to_data(prefix);
...
uint64_t /*size_t*/ chain_scriptv2_sigops(scriptv2_t script, int /*bool*/ embedded) {
    return chain_scriptv2_const_cpp(script).sigops(embedded != 0);
}

uint64_t /*size_t*/ chain_scriptv2_embedded_sigops(scriptv2_t script, scriptv2_t prevout_script) {
    auto const& prevout_script_cpp = chain_scriptv2_const_cpp(prevout_script);
    return chain_scriptv2_const_cpp(script).embedded_sigops(prevout_script_cpp);
}

//void chain_scriptv2_find_and_delete(scriptv2_t script, const data_stack& endorsements) {
//    return chain_scriptv2_const_cpp(script).embedded_sigops(find_and_delete);
//}
